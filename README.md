...
我在上面养了一套个人交付系统：多步任务强制走搜、规格、执行、证据核验和行为进化；五个常驻分身拆开决策、质检、外搜、识图和远端；知识分四库，长文只有一个现行版；改自己的规则也要过评测。没有魔改客户端。全是 `CLAUDE.md`、skills、agents、hooks、MCP。
...
我没发明第二种 Claude Code。差距在纪律、分工、记忆怎么分、以及「我觉得好了」算不算完成。
...
| 10 | 远程触角 + 长项目骨架 | 会话一关就断 |
| 11 | 改 harness 前后跑评测 | 改规则靠感觉 |
| 12 | 固定汇报骨架，对人说话有标准 | 文风和结构飘 |
...
先讲你得到什么、怎么自己验，再堆路径。  
对话默认像真人同事。  
你说「比如 A、B」会先扩同类，不修两个样本就完。  
你要详细调查时，给机制和可抄步骤，不甩简介交差。  
交付结构固定：问题→解法正文，文末总结表 + checklist。
...
公开说明仓，以 README 与导图为主。思想可复现，私人库不公开。能力面稳定后会偶尔改这篇，不保证日更。
